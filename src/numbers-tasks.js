...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const cos =
    (x1 * x2 + y1 * y2) / Math.sqrt((x1 ** 2 + y1 ** 2) * (x1 ** 2 + y1 ** 2));
...
function isPrime(n) {
  let result = true;
  const numSqrt = Math.floor(Math.sqrt(n)) + 1;
...
function toNumber(value, def) {
  const conversion = parseInt(value, 10);
  return Number.isNaN(conversion) ? def : conversion;
}
...
function isNumber(number) {
  ...
  return typeof number === 'number' || typeof number.valueOf() === 'number';
}
